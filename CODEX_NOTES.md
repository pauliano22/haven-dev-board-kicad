...
**Surfaced a real functional concern rather than silently shipping past it**: of the 15 remaining unconnected items, five are on U15 (the ADAU1860 audio codec) -- `SCL1`/`SDA1` (its I2C control bus), `LRCLK` (I2S clock), `XTALI` (crystal input), `DAC_ENABLE`. These are BGA balls, not bodge-wireable after fab. Losing the I2C pair means the codec's registers can't be configured at all; losing LRCLK+XTALI breaks its clock domain -- between them this likely means no working audio path as currently laid out, not a cosmetic gap. Checked whether a U15 placement nudge is newly viable now that the board has real surrounding space (~8mm clear on every side, unlike the original cramped footprint) -- geometrically yes, these six items (all blocked by copper *outside* U15's own footprint, unlike the two same-package internal pairs) could plausibly open up. But moving a placed footprint in `pcbnew` does not drag its existing routed traces with it -- all ~40 of U15's already-verified-clean connections would need re-terminating or full re-routing, a large, higher-risk undertaking on the scale of redoing most of this session's U15 work, not a quick nudge. Flagged to the user for a decision rather than attempted unilaterally.
