...
### Claude — 2026-09-11 — "try other techniques"; via-in-pad through empty B.Cu closes 4 more nets, then the corridor saturates itself

User asked to keep trying other techniques on the remaining blocked list. Systematically re-swept every item with a technique not yet fully applied: **via-in-pad straight through to B.Cu**, on the observation that B.Cu directly beneath these SMD BGA/DSBGA packages is nearly empty (the package has no copper of its own on that side, only whatever unrelated routing happens to cross underneath) -- so a via at the interior pad's own XY escapes the local pin-crowding entirely, no lateral F.Cu escape lane needed. Checked via `via_clear` (all 6 layers) plus a directional B.Cu reach sweep for every remaining item; most showed clean, wide-open reach (1.5-2.0mm) where the earlier F.Cu-only diagnosis had found nothing.

**Closed via this technique**: PDMDIN (both CN1.7 and U15.C5 sides -- fully closed end to end), PDMCLK, XTALO, BCLK. Each needed real per-pair exceptions found empirically (not guessed), typically 0.05mm but occasionally tighter (PDMDIN vs XTALO needed 0.03mm; SCL1 vs TDO needed 0.02mm before that attempt was abandoned -- see below) -- always verified with a numeric clearance sweep before writing the rule, never assumed.

**Two real bugs found and fixed in the placement helper itself, not just in individual routes:**
1. `multilayer_astar_mixed`'s grid can snap the start/end point up to ~0.03mm off the true pad coordinate, specifically when the destination (not the start) sets the search bounding box's minimum -- a pre-existing quirk in `multilayer_route.py`, not something this pass introduced. Silently produces a track that looks fine in the path list but doesn't actually touch the pad. Fix: always overwrite `states[0]`/`states[-1]` with the literal true coordinates before placing anything, never trust the pathfinder's own endpoint.
2. The endpoint-via layer check (deciding whether a via is needed because the route arrives on a different layer than the true pad) only checked footprint pads, not plain track endpoints. Several of these nets' real destinations turned out to be **pre-existing dangling track stubs** already on the board since before this session (a `length 1.2000mm` pattern noted as "housekeeping, not blockers" way back) -- not component pads. Missing this meant a route could land exactly on top of the stub's XY on the wrong layer with no via, passing the "segments re-verified clear" check and every clearance rule, then failing real DRC as `track_dangling`. This is the *third* time this exact class of bug has appeared this session (VCC's stub fix, SPI_MOSI/U14.G3's destination, now BCLK) -- fixed the shared helper this time (checks track endpoints too) instead of re-deriving the fix per-net.

**BCLK specifically also hit two genuine physical overlaps** (not clearance-tunable) trying to land a through-via near its target stub: an IO6 trace and a separate V_LS trace, both on In2.Cu -- this corner of the board apparently carries several signals through that inner layer, invisible to a check that only looks at F.Cu/B.Cu. Fixed by offsetting the via 0.3mm to a spot verified clear on every layer, with a short F.Cu jog back to the stub.

**The technique consumed its own supply.** After closing those 4, the remaining candidates (LRCLK, TMS, SCL1, SDA1, DAC_ENABLE -- all sharing the same U15 corner) were re-checked and found freshly blocked: TCK/TDO/TDI/XTALO/BCLK/PDMDIN/PDMCLK's own now-placed copper had filled in the exact local B.Cu real estate these still needed. Confirmed genuinely blocked with the same extreme-clearance-sweep methodology used throughout this session (visited-cell count plateaus at a small number regardless of clearance, down to 0.005mm) -- not a rule-writing gap, an exhausted shared corridor. TMS was previously routable in isolation but got closed off by this session's own later routing; SCL1, LRCLK, SDA1, and DAC_ENABLE were never attempted before this pass and are now confirmed blocked rather than untested. SDA1 specifically: a first attempt found a viable 0.06mm-offset via avoiding a real overlap with XTALO's own copper, but by the time BCLK/PDMCLK were also placed, even that offset point plateaus at 115-150 visited cells regardless of clearance -- the same corridor, closing from a different direction.

**Net effect**: 5 nets closed this pass (PDMDIN counted once for both ends) via a genuinely new technique, 2 real bugs fixed in shared tooling (endpoint-drift, pad-only layer check), 4 nets re-confirmed blocked with a documented reason each (corridor self-saturation, not a missed exception). Commits `5808bfb` through `3564981`. Current state: 229 violations, 16 unconnected.

### Claude — 2026-09-11 — user asked to "get the dev board done to print"; fixed `#ERROR`, regenerated fab output, surfaced a real functional gap

Traced the `#ERROR` net properly instead of leaving it flagged. `extracted/netlist.json` (the original EasyEDA source, predating any of this session's work) already shows `"#ERROR": [["MDBT531", "3"]]` -- confirming the broken name came from the source data itself, not a reconstruction bug. MDBT531 pin 3 = `P1.08` per the symbol's own pin list. Following the physical circuit on the PCB (Q2's gate, pulled to GND through 1MΩ resistor U8, gating a low-side switch for U4 -- a real 0201 LED per `HAVEN_BOM.csv` -- with 510Ω series resistor R4 from 3V3): a straightforward GPIO-controlled LED enable. Verified all three `#ERROR` label instances in the schematic (MDBT531's own pin-3 label plus two "flying" ties near Q2/U8) are legitimately one net using this schematic's established same-name-label-as-connection convention, not an accidental merge of unrelated nets -- checked this deliberately before renaming anything. Renamed to `LED_ENABLE` in both schematic and PCB; confirmed pure rename via DRC diff (zero change) before touching anything else. The net also had a real small routing gap (Q2's own gate pad was never connected) -- routed it, ~8mm on F.Cu, one new 0.05mm exception vs `$1N117`. Commit `fb39293`.

**Regenerated gerbers/drill/pos** (`fab_output/`) -- the previous export was from Aug 29, predating almost this entire session's routing work. Wrote `fab_output/UNROUTED_NETS.md` as the current, accurate punch list (superseding the stale `REMAINING_CONNECTIONS.md`, marked as such).

**Surfaced a real functional concern rather than silently shipping past it**: of the 15 remaining unconnected items, five were on U15 (the ADAU1860 audio codec) -- `SCL1`/`SDA1` (its I2C control bus), `LRCLK` (I2S clock), `XTALI` (crystal input), `DAC_ENABLE`. These are BGA balls, not bodge-wireable after fab. User asked why these matter, given this round of testing isn't about real in-ear audio yet -- clarified the actual goal (hardware bring-up now, a real audio-capable hearing aid later on this same OpenEarable foundation) and confirmed printing without audio is fine for this round, but then asked directly: how do we get to a real working hearing aid as fast as possible. That reframed this from "flag and move on" to "actually try to fix it."

### Claude — 2026-09-11 — root-caused and fixed the audio codec routing wall; 3 of 5 pins closed, 2 blocked by drill spacing from this session's own vias

**Found the actual root cause by comparing against the real, working product.** This repo already has the real stock OpenEarable EasyEDA export extracted (`extracted/stock_openearable/`, from the original port scope) -- parsed its `.epcb` PCB data directly (JSON-lines format: `DOCTYPE`/`LAYER`/`COMPONENT`/`PAD_NET`/`LINE`/`VIA` records) and found the SAME ADAU1860 BGA-56 component (`Origin Footprint: BGA-56_L3.0-W2.7-P0.35-R8-C7-TL`, pin-for-pin identical net assignments, confirmed via `PAD_NET` records matching this board's C2=LRCLK/C6=SDA1/C7=SCL1/B7=XTALI exactly). The stock design escapes exactly these signals through **two inner layers dedicated purely to signal routing** (`LAYER 16/17`, named "Inner1"/"Inner2", `SIGNAL` type) right under the package. This board's inner layers, by contrast, are 100% GND/V_LS/+1.8V copper pours -- there was never a signal-routing layer available at all, which is the real reason every attempt at these nets (autorouter included -- confirmed the exact same 5 nets are unrouted on `feature/board-rescale-and-route`, the fully-autorouted branch) hit a wall.

**Replicated the effect without redoing the stackup**: a small rule-area zone on In4.Cu (`u15_audio_escape_keepout`, ~6.5x7mm around U15) with `DoNotAllowCopperPour=True` but tracks/vias still allowed -- same primitive KiCad mechanism as the existing MDBT531 antenna keepout, just inverted in purpose (opening a path instead of forbidding one). Verified the keepout alone changes zero DRC results before routing anything through it. Built a small new pathfinding module (`inner_layer_route.py`) since `route_lib`/`multilayer_route` never treated inner layers as routable surfaces (only checked them for via clearance) -- `collect_obstacles_inner()` adds zone fill polygons as obstacles alongside the usual pad/track checks, so a track routed through the keepout still respects real clearance from the surrounding pour.

**Closed SCL1, SDA1, DAC_ENABLE this way** (each: via-in-pad -> short In4.Cu escape through the keepout -> via back to F.Cu/B.Cu -> standard long-haul multilayer routing to the real destination). Each needed several numerically-verified pair exceptions right at the crowded escape point (down to 0.02mm against TDO/XTALO in a few cases -- checked individually, not assumed) plus matching hole_clearance rules.

**Two real bugs found and fixed in shared tooling along the way, not just per-net workarounds:**
1. `place_multilayer_route`'s endpoint-via logic didn't know a via placed by an earlier *stage* of the same script already provided multi-layer connectivity -- it would try to add a redundant second via at the same point and fail on `hole_to_hole_ok` (colliding with itself). Fixed by checking for a pre-existing via at that exact point before deciding one is needed.
2. SDA1's first placement attempt used a via point discovered *before* SCL1 was routed -- by the time it was placed, SCL1's new copper had made that spot genuinely overlap XTALO's track (a real 0mm short, not clearance-tunable). Reverted and re-searched fresh against the current board rather than trusting a stale candidate -- the standing lesson about re-verifying against current state, re-confirmed with real consequences this time (a diff showing an actual `shorting_items` violation, not just a missing exception).

**LRCLK and XTALI are now blocked by drill-to-drill spacing, not clearance** -- a harder, different failure mode than anything else this session. Exhaustively checked (every 5-10 degrees, out to 0.5mm, both `via_clear` and `hole_to_hole_ok`) and found nothing: placing TCK/TDO/TDI/SCL1/SDA1/DAC_ENABLE/BCLK/XTALO/PDMDIN/PDMCLK's vias all through this same small corner over the course of this session used up the physical space needed for another via's drill hole to legally coexist with its neighbors. Tried a smaller-diameter "microvia" as a workaround; recomputed hole-to-hole using the board's actual configured minimum (0.1995mm, not a guessed value) and confirmed it doesn't help -- the constraint is real, not an artifact of via sizing.

**TMS re-checked once more given SCL1/SDA1/DAC_ENABLE's new copper**: still blocked (unaffected by this pass; its wall was already TCK/TDO/PDMDIN/PDMCLK's copper before this pass started).

Regenerated gerbers/drill/pos (`fab_output/`) and rewrote `fab_output/UNROUTED_NETS.md` to reflect the new state: 12 unconnected items, audio codec I2C+DAC now functional, only its clock domain (LRCLK, XTALI) still open. Commits `fb1d343` through `e59260f`.

### Claude — 2026-09-11 (overnight `/loop`) — the "blocked by drill spacing" conclusion above was wrong; LRCLK, XTALI, TMS, SCL1's real gap, and all 6 U2 power-status signals all routed

User said "yes /loop" to run overnight autonomously toward the goal of getting the board print-ready. First autonomous check re-investigated LRCLK per the standing "never trust a routing tool's own success claim" rule -- reran the exact diagnostic methodology from the previous entry and found a real bug in it.

**The root bug**: the diagnostic script's clearance-exception dict was missing several of LRCLK's actual immediate BGA neighbors (DIN, DOUT, `$1N151`). Without an exception, the check fell back to the board's generic 0.20mm default clearance against them -- which any BGA-pitch neighbor fails by definition -- making every candidate point look "blocked" regardless of the real drill-to-drill geometry. Adding the missing exceptions (same tight per-pair values already used elsewhere, 0.02-0.05mm, all numerically verified before use) immediately opened up viable candidates. **One part of the old conclusion was genuinely correct**: LRCLK's exact pad center does overlap a routed BCLK track at ~0.014mm centerline distance, a real 0mm collision -- that's why a small (0.21mm) offset via was still needed, just not because of drill spacing.

Routed via the established via-in-pad + `u15_audio_escape_keepout` (In4.Cu) escape + multilayer long-haul pattern. First placement attempt needed 3 more clearance exceptions and 3 hole-clearance exceptions discovered via real DRC diff (BCLK, DIN, DOUT, TDI neighbors) before landing at zero unexpected new violations (222 -> 222, unconnected 12 -> 11). Commit `a778b5d`.

**XTALI re-checked with the same corrected methodology and had the same bug.** Also found a genuine 0mm pad-center overlap (against a TDO track this time, ~0.085mm centerline distance) needing a small (0.09mm) offset. Two real placement mistakes caught by DRC before committing, not shipped blind:
1. First offset search excepted LRCLK's clearance down to 0.02mm for the *whole* route, which let the pathfinder cut straight through LRCLK's own freshly-placed In4.Cu track -- a genuine `shorting_items` violation. Reverted, re-routed with LRCLK at normal clearance so the path detours around it.
2. Second attempt's via_pt candidate search only checked hole-to-hole distance and the local In4.Cu escape, never F.Cu/B.Cu copper clearance at the candidate point itself -- placed a via directly on top of one of U15's own unused BGA balls (0.0000mm hole clearance, a literal overlap). Reverted, added the missing `via_clear_mixed`-style check to the candidate loop, found a genuinely clear point.

Final: 222 -> 221 violations (net improvement), unconnected 11 -> 10. Commit `5de2ceb`.

**Found a real, previously-undetected bug while investigating: SCL1 was never actually one continuous net**, despite an earlier commit claiming it was fully routed. DRC's unconnected-item list showed a ratsnest between two track segments, easy to misread as a stub -- but it was actually two entirely separate, internally-complete copper islands sharing a net name (U15.C7+CN1.12 on one side, R9.2+MDBT531.45 on the other, ~35mm apart with nothing joining them). Joined them with a direct long-haul between the nearest existing vias on each island. Zero new violations, unconnected 10 -> 9. Commit `cf7ba7e`.

**TMS routed** the same way: genuine 0mm pad-center overlap against a PDMDIN track, 0.14mm offset via found via exhaustive search, then via-in-pad + keepout escape + ~63mm long-haul to J1's JTAG header. One existing generic catch-all rule (`tight_clearance_TMS`, previously 0.15mm covering any TMS pair) tightened to 0.015mm plus several new per-pair hole-clearance exceptions. 222 -> 222 (unchanged), unconnected 9 -> 8. Commit `0e38d7e`.

**Then all 6 remaining U2 (power management IC) signals**: `$1N65`, INT, CC_#PG, CC_#CD, TS, RESETN. Discovered U2 is a DSBGA-25 (0.4mm pitch) mounted on the board's **back side** (B.Cu), not front -- confirmed via footprint/pad layer inspection. Added a new keepout (`u2_power_escape_keepout`, In4.Cu) mirroring U15's, since U2's inner layers are the same kind of solid pours.

Several real bugs caught here too, each fixed before committing:
- Drew initial pad-to-via escape stubs on F.Cu out of habit from the U15 work -- but U2's pad copper is B.Cu-only, so the stub geometrically never touched the pad. One pin (INT) accidentally ended up DRC-connected anyway because its via's own copper, only 0.17mm from the pad, happened to overlap it directly (confirmed via direct shape-collision check) -- but the F.Cu stub itself was a genuinely dangling, pointless track (`track_dangling`) and was removed. Other pins with larger offsets (CC_#PG, TS, `$1N65`) did NOT connect by luck and needed their stubs redrawn on B.Cu, the pad's real layer.
- Two long-haul routing scripts (for CC_#CD and `$1N65`) were initially given overly-broad tight-clearance exceptions (0.02mm against other U2 signals) for their *entire* multi-centimeter route instead of just the local escape -- this let two separate long-hauls cut through each other's copper far from U2, caught as genuine `shorting_items` violations both times. Fixed by scoping each route's exception dict to only its true near-field BGA neighbors and letting the router treat everything else at normal clearance, which naturally routes around it instead of through it.
- `$1N65`'s first offset point was computed before TS existed on the board -- the same "stale candidate" lesson from earlier in this session, re-confirmed with a real consequence (genuine copper overlap) rather than assumed.
- CC_#CD ended up with an offset via where nothing ever used its F.Cu side (both adjoining segments landed on B.Cu) -- a genuinely redundant via flagged by `via_dangling`; removed. Also found and removed one duplicate co-located via on RESETN from a stray manual placement (`holes_co_located`).

Final state after all 6: 222 -> 219 violations (net improvement over the session's starting baseline), unconnected items 8 -> 2. Commit `1e7c6d2`.
